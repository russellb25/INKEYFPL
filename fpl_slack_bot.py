...
# ------------ Config via env vars ------------
LEAGUE_ID = os.getenv("FPL_LEAGUE_ID")  # e.g. "215925"
SLACK_WEBHOOK_URL = os.getenv("SLACK_WEBHOOK_URL")  # Incoming webhook or Workflow Trigger
POST_CHANNEL = os.getenv("SLACK_CHANNEL", "")  # optional, if your webhook permits overriding
TEAM_LIMIT = int(os.getenv("TEAM_LIMIT", "50"))  # how many teams to scan
...
session.headers.update({"User-Agent": "INKEY FPL Slack Bot"})
...
def current_phase_from_date(events: List[Dict[str, Any]], now_utc: datetime) -> int:
    # Use the phase of the latest finished GW
...
def build_overall_table(league_id: str, limit: int) -> List[Tuple[str, int]]:
    ...
    ranked = sorted(
        [(f"{t['entry_name']} ({t['player_name']})", int(t["total"])) for t in teams],
        key=lambda x: x[1],
        reverse=True,
    )
    return ranked
...
def build_month_table(league_id: str, phase: int, limit: int) -> List[Tuple[str, int]]:
    ...
    ranked = sorted(
        [(f"{t['entry_name']} ({t['player_name']})", int(t["event_total"])) for t in teams],
        key=lambda x: x[1],
        reverse=True,
    )
    return ranked
...
def post_to_slack_workflow_trigger(text: str, context: Dict[str, Any]) -> None:
    """
    Slack Workflow Builder webhook trigger needs flat key-value strings.
    You format the message inside the workflow using these variables.
    """
    flat_payload = {k: str(v) for k, v in context.items()}
    flat_payload.setdefault("text", text)
    r = session.post(SLACK_WEBHOOK_URL, json=flat_payload, timeout=30)
    if r.status_code >= 300:
        logging.error("Slack workflow trigger error %s: %s", r.status_code, r.text)
        r.raise_for_status()
...
def main() -> None:
    ...
    if "hooks.slack.com/triggers/" in SLACK_WEBHOOK_URL:
        context = {
            "header": header,
            "monthly_table": fmt_table(monthly, top_n=10),
            "overall_table": fmt_table(overall, top_n=10),
            "bottom_text": f"Bottom of the week: {worst_name} with {worst_points} pts",
            "gw_name": gw_name,
            "phase_name": phase_name,
        }
        post_to_slack_workflow_trigger(text=header, context=context)
    else:
...
if __name__ == "__main__":
    try:
        ...
        # Try to alert in Slack too
        try:
            if "hooks.slack.com/triggers/" in os.getenv("SLACK_WEBHOOK_URL", ""):
                post_to_slack_workflow_trigger(text=f"FPL bot failed: {e}", context={"error": str(e)})
            else:
